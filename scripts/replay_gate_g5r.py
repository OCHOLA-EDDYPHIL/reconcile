#!/usr/bin/env python3
"""Validate and present the sanitized final-candidate proof bundle."""
...
def _validate_provider(payload: dict[str, Any]) -> None:
    ...
    _require(
        ...
        "provider proof schema changed",
    )
    _require(provider["status"] == "PASS", "provider proof status is not PASS")
...
def load_and_validate(path: Path) -> dict[str, Any]:
    """Load and cross-check the four-file final-candidate evidence bundle."""
    ...
    for label, raw, expected in (
        ("provider proof", provider_raw, references["provider_proof_sha256"]),
...
def _print_human(payload: dict[str, Any]) -> None:
    ...
    provider = payload["provider_proof"]
    print("RECONCILE — Proof-to-Permit offline evidence validation")
    ...
    print("  evidence     -> 49 durable events; provider/live projection hash linked")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate and present the sanitized Proof-to-Permit evidence bundle."
    )
