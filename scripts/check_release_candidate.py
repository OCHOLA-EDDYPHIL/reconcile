#!/usr/bin/env python3
"""Focused, local acceptance for the documentation and demo candidate."""
...
def _check_claim_markers() -> None:
    ...
    _require("no current public endpoint" in readme, "endpoint status is missing")
...
def _check_graphviz() -> str:
    ...
    if executable is None:
        ...
        return "exported SVG structure checked (Graphviz unavailable)"

    with tempfile.TemporaryDirectory(prefix="reconcile-diagram-check-") as directory:
        temporary = Path(directory)
        for source, export in (
            (ROOT / "docs" / "architecture.dot", ROOT / "docs" / "architecture.svg"),
            (ROOT / "demo" / "proof.dot", ROOT / "demo" / "proof.svg"),
        ):
            rendered = temporary / export.name
            subprocess.run(
                [executable, "-Tsvg", str(source), "-o", str(rendered)],
                check=True,
...
def main() -> int:
    ...
    print("RECONCILE release candidate: PASS")
