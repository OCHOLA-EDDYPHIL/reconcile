...
RUN set -eu; \
    ...
    metadata=/opt/reconcile/lib/python3.12/site-packages/reconcile-0.1.0.dist-info; \
    record="$metadata/RECORD"; \
    cache="$metadata/uv_cache.json"; \
    test -f "$cache"; \
    test "$(grep -F -c 'reconcile-0.1.0.dist-info/uv_cache.json,' "$record")" = 1; \
    rm "$cache"; \
    sed -i '/^reconcile-0[.]1[.]0[.]dist-info\/uv_cache[.]json,/d' "$record"
